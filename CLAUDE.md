...
### worker側の必須項目チェック＋控えメール体制（2026-07-11導入）

- **必須チェック**：`handleNyukai`＝氏・名・教室名・初回授業日（＋YYYY-MM-DD形式）／`handleTaiken`＝氏・名・メールアドレス・教室名・希望日時。未入力は400で登録拒否
  - ⚠️ 必須に追加してよいのは「東京直営＋FC加盟店の**全フォームが必ず送る項目**」のみ（workerは全組織共通のため）
- **控えメール**：体験・入会の登録成功時、全組織分の申込内容を `k-ariyama@alpha-brain.jp`（`ADMIN_EMAIL`）へ自動送信（Gmailが控え台帳）
- **失敗時セーフティネット**：kintone登録失敗時は申込内容全文＋エラーを【要対応】メールで同宛先へ通知してからエラー返却（リード取りこぼし防止）。`sendAdminEmail` は throw しない設計で、メール障害が申込処理を巻き込まない
...
> **注意（2026-08-31・TOKEN_KENTEIで発生）**: kintone APIトークンは同じアプリでも「レコード閲覧」「レコード追加」「レコード編集」「レコード削除」を個別に権限設定できる。POST専用で使われてきたトークン（例: 登録専用の `TOKEN_KENTEI`）を使って新規に GET 一覧取得エンドポイントを追加すると、`このAPIトークンでは、指定したAPIを実行できません` エラーになることがある。**既存トークンを使ってGETハンドラを新規追加するときは、kintone側でそのトークンに「レコード閲覧」権限があるか事前に確認・依頼すること。**
...
| 振替受講教室 | △ | 教室セレクタ（3校）。「未定」チェックで非表示。未定でない場合は必須（2026-08-28〜） |
| 振替受講日 | △ | 「未定」チェックで非表示。未定でない場合は必須（2026-08-28〜） |
| 時刻 | 任意 | 教室連動プルダウン |
| 備考 | 任意 | 自由記入 |
